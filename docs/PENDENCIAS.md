...
|---|---|---|
| **22** | 🟠 **A primeira consulta real ao provedor ainda nao foi observada** (19/08/2026, 22h50) | O conector esta testado na LOGICA (7 testes com `fetch` controlado), nao contra a fonte viva: a saida de rede desta maquina nao alcancou o provedor — uma tentativa ficou pendurada dez minutos. **Nao afirmo que a consulta real funciona.** Falta rodar uma vez de um lugar com saida de rede e registrar o resultado: qual provedor respondeu, quanto tempo levou, e se o formato bate com a normalizacao |
