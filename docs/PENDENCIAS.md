...
| **20** | 🟠 **O site está no ar SEM as variáveis do Supabase** (19/08/2026, 21h40) | https://csi-brasil.vercel.app responde 200, mas `NEXT_PUBLIC_SUPABASE_URL` e `NEXT_PUBLIC_SUPABASE_ANON_KEY` **não existem no projeto Vercel** — não há projeto Supabase. ⚠️ **O build passa assim mesmo**, porque a página atual não toca no banco. Isso é verde que não prova nada sobre o encanamento de dados: **no dia em que a primeira tela ler o banco, o deploy quebra** se elas não estiverem cadastradas. Cadastrar em Vercel → Settings → Environment Variables, junto com a criação do projeto Supabase (item 19) |
| **19** | 🔴 **AS TRÊS MIGRATIONS ESTÃO ESCRITAS E NÃO APLICADAS** (19/08/2026, 21h15) | Dívida assumida de propósito pelo dono: *"escreve as migrations agora, eu aplico depois"*. `0001_organizations.sql`, `0002_projects.sql` e `0003_monitors.sql` existem no repositório e **nunca tocaram um banco** — não há projeto Supabase. ⚠️ **Enquanto isso durar, o repositório descreve um banco que não existe.** É exatamente a situação que no projeto anterior deixou um alerta de segurança no ar por dias, e que fez o histórico de migrations sair de sincronia a ponto de `db push` virar operação perigosa. **Ao aplicar, conferir cada uma — "aplicou" e "funciona" são coisas diferentes:** (a) `select tablename, rowsecurity from pg_tables where schemaname='public'` → todas com `rowsecurity = true`; (b) com **dois usuários de organizações diferentes**, contar `projects` em cada sessão → cada um só vê o seu; se contar os dois, a RLS não está cortando e **nada mais pode seguir**; (c) tentar `update` numa `query_versions` → **zero linhas afetadas**, e ⚠️ o Postgres **não devolve erro** nesse caso: conferir contando as linhas, não pelo status. **Também falta a extensão `pgvector`** — decidida em 19/08, mas nenhuma tabela da Fase 1 usa vetor, então ela entra junto com a primeira coluna de embedding, na Fase 2 |
