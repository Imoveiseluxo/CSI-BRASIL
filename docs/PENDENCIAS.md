...
| 14 | 🆕 **O Book v2 mudou o modelo de dados, não só o roadmap** (19/08/2026) | O v2 escreve como regra de projeto: *"o grafo não é decoração. Cada nó e cada aresta devem ser rastreáveis a evidência, fonte, Transform, usuário/agente responsável, data e nível de confiança."* Isso **não é feature da Fase 7** — é coluna obrigatória em toda tabela que guarde nó ou aresta, desde a primeira. **O plano da Fase 1 foi escrito sobre o v1 e não tem isso.** Precisa ser relido antes de executar, senão o modelo nasce sem linhagem e reescrever depois custa a base inteira. Mitigação: revisar o plano da Fase 1 acrescentando `origem`, `evidência`, `confiança` e `quem/como gerou` no molde de tabela de domínio |
